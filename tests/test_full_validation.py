...
def test_complete_corpus_baseline_is_deterministic_and_consistent(
    ...
    assert first.aggregates.exact_matches == 1442
    assert first.aggregates.mismatches == 2
    ...
    assert sum(count for _, count in first.aggregates.mismatch_status_counts) == 2
