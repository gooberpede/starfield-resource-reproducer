...
def test_production_scaled_index_matches_algorab_live_descendant_result() -> None:
    rng = StarfieldRng(1654436101)
    for _ in range(17):
        rng.next_uint32()

    actual = rng.next_scaled_index(2)
    assert rng.draw_count == 18
    assert rng.last_draw is not None
    assert rng.last_draw.raw_value == 1826241303
    assert rng.last_draw.probability_value == 0.4252006709575653
    assert rng.last_draw.scaled_value == 0.8504013419151306
    assert actual == 0
